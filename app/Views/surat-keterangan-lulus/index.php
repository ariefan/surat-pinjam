...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-12">
                <h1 class="m-0">Surat Keterangan Lulus</h1>
            </div>
        </div>
    </div>
</div>
<div class="content">
    <div class="container-fluid">
            ...
            <div class="col-lg-12">
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-success" title="Tambah" id="btnTambahST"
                        href="<?= site_url('suratketeranganlulus/create'); ?>">Tambah</a>
                <?php } ?>
            </div>
        </div>


        <form action="<?= site_url("suratketeranganlulus/index"); ?>" method="post">
            <div class="form-group row">
                <div class="col-sm-2">
                    <input id="tanggal_pengajuan" onload="getDate()" type="date" name="date" class="form-control"
                        placeholder="Pencarian" value="<?= $date ?? date("Y-m-d"); ?>">
                </div>
                <div class="col-sm-4">
                    <input type="text" name="q" class="form-control" placeholder="No Surat" value="<?= $q; ?>"
                        autocomplete="off">
                    <input type="hidden" name="sort_column" value="<?= $sort_column; ?>">
                    <input type="hidden" name="sort_order" value="<?= $sort_order; ?>">
                </div>
                <div class="col-sm-2">
                    <button class="btn btn-success" title="Cari" type="submit">Cari</button>
                </div>
            </div>
        </form>
        ...
        <form method="POST">
            ...
            <?php if (in_array($jenis_user, ['dekan', 'wadek', 'verifikator'])) { ?>
                <button
                    onclick="return confirm('Apakah anda yakin ingin approve surat-surat ini? \nPASTIKAN SUDAH PREVIEW TERLEBIH DAHULU');"
                    formaction="<?= base_url("suratketeranganlulus/approvedeclinemultiple/") ?>" type="submit"
                    value="<?= in_array($jenis_user, ['dekan', 'wadek', 'verifikator']) ? 3 : 2; ?>" name="approveOrDecline"
                    class="btn btn-sm btn-primary tombolApproveBanyak">
                    Approve Banyak
                </button>
                <button formaction="<?= base_url("suratketeranganlulus/approvedeclinemultiple/") ?>" type="submit" value=-1
                    name="approveOrDecline" class="btn btn-sm btn-primary tombolDeclineBanyak">
                    Decline Banyak
                </button>

            <?php } ?>

            <button formaction="<?= base_url("suratketeranganlulus/topdfbatch/") ?>" formtarget="_blank" value="id"
                type=" submit" name="preview" class="btn btn-sm btn-primary tombolPreviewBanyak">
                Preview/Print-ID
            </button>
            <button formaction="<?= base_url("suratketeranganlulus/topdfbatch/") ?>" formtarget="_blank" value="en"
                type="submit" name="preview" class="btn btn-sm btn-primary tombolPreviewBanyak">
                Preview/Print-EN
            </button>

            <?php if (in_array($jenis_user, ['verifikator', 'admin'])) ?>
            <button onclick="return confirm('Apakah anda yakin ingin menghapus banyak surat?');"
                formaction="<?= ("suratketeranganlulus/deletemultiple/") ?>" type="submit" name="delete"
                class="btn btn-sm btn-danger  tombolHapusBanyak">
                Hapus
            </button>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        $print_header = function ($label, $column_name, $q) use ($sort_column, $sort_order) {
                            $url = site_url('suratketeranganlulus/index') . "?q=$q&sort_column=$column_name&sort_order=" . ($sort_order == 'asc' ? 'desc' : 'asc');
                        ...
                        <table class="table table-bordered table-valign-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        <a
                                            href="<?= site_url('suratketeranganlulus/index'); ?>?q=<?= $q; ?>&sort_column=nama&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Pengaju</a>
                                        <i
                                            class="pl-2 <?= $sort_column == 'nama' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                    </th>
                                    <th>
                                        <a
                                            href="<?= site_url('suratketeranganlulus/index'); ?>?q=<?= $q; ?>&sort_column=tanggal_pengajuan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Tanggal
                                            Pengajuan</a>
                                        <i
                                ...
                                <?php } ?> -->
                                    <th>
                                        <a
                                            href="<?= site_url('suratketeranganlulus/index'); ?>?q=<?= $q; ?>&sort_column=status&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Status</a>
                                        <i
                            ...
                            <tbody>
                                <?php foreach ($rows as $row) { ?>
                                    ...
                                    $akses = [
                                        ...
                                        'upload_pj' => '',
                                        // 'preview' => (($row->status == 1 && in_array($jenis_user, ['verifikator'])) || ($row->status == 2 && in_array($jenis_user, ['dekan', 'wadek']))) && !empty($row->no_surat) ? 1 : 0,
                                        'share' => '',
                                        // 'approve' => ($row->status == 1 && in_array($jenis_user, ['verifikator'])) || ($row->status == 2 && in_array($jenis_user, ['dekan', 'wadek'])) ? '' : 'disabled',
                                        // Aktifkan ini jika ingin verifikator bisa approve
                                    ...
                                    <tr>
                                        <th>
                                            <input type="checkbox" name="checkbox_value[]" id="centangSemua" class="centang"
                                                <?= $row->status <= 1 ? '' : 'disabled' ?> value=<?= $row->id ?> />
                                        </th>
                                        <td>
                                            ...
                                            <div class="btn-group btn-group-sm" role="group">
                                                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])): ?>
                                                    <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit"
                                                        href="<?= base_url("suratketeranganlulus/edit/" . $row->id); ?>"><i
                                                            class="fa-solid fa-pencil"></i></a>
                                                    <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete"
                                                        href="<?= base_url("suratketeranganlulus/delete/" . $row->id); ?>"
                                                ...
                                                <a class="btn btn-success <?= $akses['download']; ?>" title="download"
                                                    href="<?= base_url("suratketeranganlulus/topdf/" . $row->id); ?>"
                                                    target="__blank"><i class="fa-solid fa-eye"></i></a>
                                                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])): ?>
                                                    <!-- <a class="btn btn-primary btn-sm <?= $akses['upload_pj']; ?>" title="Unggah Laporan Pertanggungjawaban" href="<?= base_url("/suratketeranganlulus/upload/" . $row->id); ?>"><i class="fa-solid fa-upload"></i></a> -->
                                                    ...
                                                    <a class="btn btn-outline-primary btn-sm <?= $akses['share']; ?>"
                                                        title="Bagikan"
                                                        href="<?= base_url("suratketeranganlulus/share/" . $row->id); ?>"><i
                                                            class="fa-solid fa-share"></i></a>
                                                <?php endif ?>
                                                <a class="btn btn-info btn-sm button-preview" title="Lihat dan Setujui"
                                                    href="#"
                                                    onclick="viewpdf('<?= $row->id; ?>', <?= $akses['preview']; ?>)"
                                                    data-komentar="<?= $row->komentar; ?>"
                                                    data-="<?= $akses['approve']; ?>"><i
                    ...
                    <div class="col-sm-3" id="preview-panel">
                        ...
                        <form id="form-komentar" style="float:left;" action="" method="POST">
                            ...
                            <div class="form-group">
                                <button class="btn btn-sm btn-primary" type="submit">Komentari</button>
                            </div>
                            <div class="form-group" style="position: absolute;bottom:0;">
                                ...
                                <!-- <button type="submit" name="status" value="" id="approve" class="btn btn-success" onclick="return confirm('Apakah anda yakin ingin menyetujui surat ini?');">Approve</button> -->
                                <!-- Aktifkan ini jika verifikator bisa menyetujui surat -->
                                <button type="submit" name="status"
                                    value="<?= in_array($jenis_user, ['dekan', 'wadek', 'verifikator']) ? 3 : 2; ?>"
...
<script>
    ...
    function viewpdf(id, akses) {
        ...
        // $('#form-komentar').attr('action', "<?= site_url('suratketeranganlulus/update/') ?>" + id);
        if (akses) {
            $('#preview-panel').show()
        } else {
            $('#preview-panel').hide()
        }
        var url = "<?= site_url('suratketeranganlulus/topdf/'); ?>" + id;
