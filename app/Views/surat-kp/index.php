...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-12">
                <h1 class="m-0">Surat Kerja Praktik/Izin</h1>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="col-lg-12">
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-success" title="Tambah" id="btnTambahST"
                        href="<?= site_url('suratkp/create'); ?>">Tambah</a>
                <?php } ?>
            </div>
        </div>

        <form action="<?= site_url("suratkp/index"); ?>" method="get">
            <div class="form-group row">
                <div class="col-sm-8">
                    ...
                    <table class="table table-bordered table-valign-middle">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=nama_surat&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Nama
                                        Surat</a>
                                    <i
                                ...
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=peserta&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Nama
                                        Mahasiswa</a>
                                    <i
                                        class="pl-2 <?= $sort_column == 'peserta' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'peserta' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=lokasi_kegiatan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Lokasi
                                        Kegiatan</a>
                                    <i
                                        class="pl-2 <?= $sort_column == 'lokasi_kegiatan' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'lokasi_kegiatan' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=nama&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Pengaju</a>
                                    <i
                                        class="pl-2 <?= $sort_column == 'nama' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=tanggal_pengajuan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Tanggal
                                        Pengajuan</a>
                                    <i
                                ...
                                <th>
                                    <a
                                        href="<?= site_url('suratkp/index'); ?>?q=<?= $q; ?>&sort_column=status&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Status</a>
                                    <i
                        ...
                        <tbody>
                            <?php foreach ($rows as $row) { ?>
                                ...
                                <tr>
                                    ...
                                    <!-- <td>
                                        <?= json_decode($row->peserta)[0]->nama; ?>
                                    </td> -->
                                    <td>
                                        <?php
                                        $peserta = json_decode($row->peserta);
                                    ...
                                    <td>
                                        <?= $row->tanggal_kegiatan_mulai; ?> s/d
                                        <?= $row->tanggal_kegiatan_selesai; ?>
                                    </td>
                                    <td
                                        ...
                                        <div class="btn-group btn-group-sm" role="group">
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])): ?>
                                                <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit"
                                                    href="<?= base_url("suratkp/edit/" . $row->id); ?>"><i
                                                        class="fa-solid fa-pencil"></i></a>
                                                <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete"
                                                    href="<?= base_url("suratkp/delete/" . $row->id); ?>"
                                            ...
                                            <a class="btn btn-success <?= $akses['download']; ?>" title="download"
                                                href="<?= base_url("suratkp/topdf/" . $row->id); ?>" target="__blank"><i
                                                    class="fa-solid fa-eye"></i></a>
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])): ?>
                                                <!-- <a class="btn btn-primary btn-sm <?= $akses['upload_pj']; ?>" title="Unggah Laporan Pertanggungjawaban" href="<?= base_url("/suratkp/upload/" . $row->id); ?>"><i class="fa-solid fa-upload"></i></a> -->
                                                ...
                                                <a class="btn btn-outline-primary btn-sm <?= $akses['share']; ?>"
                                                    title="Bagikan" href="<?= base_url("suratkp/share/" . $row->id); ?>"><i
