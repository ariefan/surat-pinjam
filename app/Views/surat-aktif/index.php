        ...
        <div class="form-group row">
            <div class="col-lg-12">
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('surataktif/create'); ?>">Tambah</a>
                <?php } ?>
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk Pembuatan Surat Aktif" href="<?= base_url('/user_manual/buat_surat_tugas.pdf'); ?>">Petunjuk Pembuatan Surat Aktif</a>
                <?php } ?>
                <?php if (in_array($jenis_user, ['verifikator', 'admin'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk Verifikasi Surat Aktif" href="<?= base_url('/user_manual/verifikasi_surat_tugas.pdf'); ?>">Petunjuk Verifikasi Surat Aktif</a>
                <?php } ?>
                <?php if (in_array($jenis_user, ['dekan', 'wadek', 'admin'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk ttd Surat Tugas" href="<?= base_url('/user_manual/ttd_surat_tugas.pdf'); ?>">Petunjuk ttd Surat Aktif</a>
                <?php } ?>
            </div>
        </div>

        <form action="<?= site_url("surataktif/index"); ?>" method="get">
            <div class="form-group row">
                <div class="col-sm-8">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    $print_header = function ($label, $column_name, $q) use ($sort_column, $sort_order) {
                        $url = site_url('surataktif/index') . "?q=$q&sort_column=$column_name&sort_order=" . ($sort_order == 'asc' ? 'desc' : 'asc');
                    ...
                    <table class="table table-bordered table-valign-middle">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <a href="<?= site_url('surataktif/index'); ?>?q=<?= $q; ?>&sort_column=nama&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Pengaju</a>
                                    <i class="pl-2 <?= $sort_column == 'nama' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a href="<?= site_url('surataktif/index'); ?>?q=<?= $q; ?>&sort_column=tanggal_pengajuan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Tanggal Pengajuan</a>
                                ...
                                <?php } ?> -->
                                <th>
                                    <a href="<?= site_url('surataktif/index'); ?>?q=<?= $q; ?>&sort_column=status&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Status</a>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("surataktif/edit/" . $row->id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                                <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("surataktif/delete/" . $row->id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                            <?php endif ?>
                                            <a class="btn btn-success <?= $akses['download']; ?>" title="download" href="<?= base_url("surataktif/topdf/" . $row->id); ?>" target="__blank"><i class="fa-solid fa-eye"></i></a>
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <!-- <a class="btn btn-primary btn-sm <?= $akses['upload_pj']; ?>" title="Unggah Laporan Pertanggungjawaban" href="<?= base_url("/surataktif/upload/" . $row->id); ?>"><i class="fa-solid fa-upload"></i></a> -->
                                                ...
                                                <a class="btn btn-outline-primary btn-sm <?= $akses['share']; ?>" title="Bagikan" href="<?= base_url("surataktif/share/" . $row->id); ?>"><i class="fa-solid fa-share"></i></a>
                                            <?php endif ?>
...
<script>
    function viewpdf(id, akses) {
        ...
        $('#form-komentar').attr('action', "<?= site_url('surataktif/update/') ?>" + id);
        if (akses) {
            $('#preview-panel').show()
        } else {
            $('#preview-panel').hide()
        }
        var url = "<?= site_url('surataktif/topdf/'); ?>" + id
