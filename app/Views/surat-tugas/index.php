        ...
        <div class="form-group row">
            <div class="col-lg-12">
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('surattugas/create'); ?>">Tambah</a>
                <?php } ?>
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk Pembuatan Surat Tugas" href="<?= base_url('/user_manual/buat_surat_tugas.pdf'); ?>">Petunjuk Pembuatan Surat Tugas</a>
                <?php } ?>
                <?php if (in_array($jenis_user, ['verifikator', 'admin', 'departemen'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk Verifikasi Surat Tugas" href="<?= base_url('/user_manual/verifikasi_surat_tugas.pdf'); ?>">Petunjuk Verifikasi Surat Tugas</a>
                <?php } ?>
                <?php if (in_array($jenis_user, ['dekan', 'wadek', 'admin'])) { ?>
                    <a class="btn btn-outline-primary" title="Download Petunjuk ttd Surat Tugas" href="<?= base_url('/user_manual/ttd_surat_tugas.pdf'); ?>">Petunjuk ttd Surat Tugas</a>
                <?php } ?>
            </div>
        </div>

        <form action="<?= site_url("surattugas/index"); ?>" method="get">
            <div class="form-group row">
                <div class="col-sm-8">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    $print_header = function ($label, $column_name, $q) use ($sort_column, $sort_order) {
                        $url = site_url('surattugas/index') . "?q=$q&sort_column=$column_name&sort_order=" . ($sort_order == 'asc' ? 'desc' : 'asc');
                    ...
                    ?>
                    <table class="table table-responsive table-bordered table-valign-middle">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th><?= $print_header('No Surat', 'no_surat', $q); ?></th>
                                <th>
                                    <a href="<?= site_url('surattugas/index'); ?>?q=<?= $q; ?>&sort_column=nama_surat&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Nama Surat</a>
                                    <i class="pl-2 <?= $sort_column == 'nama_surat' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama_surat' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a href="<?= site_url('surattugas/index'); ?>?q=<?= $q; ?>&sort_column=nama&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Pengaju</a>
                                    <i class="pl-2 <?= $sort_column == 'nama' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a href="<?= site_url('surattugas/index'); ?>?q=<?= $q; ?>&sort_column=tanggal_pengajuan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Tanggal Pengajuan</a>
                                ...
                                <th>Periode Surat Tugas</th>
                                <th>
                                    <a href="<?= site_url('surattugas/index'); ?>?q=<?= $q; ?>&sort_column=status&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Status</a>
                        ...
                        <tbody>
                            <?php foreach ($rows as $row) { ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?= $row->nama_surat; ?><?= $row->tanggal_kegiatan_selesai < date('Y-m-d') && !file_exists('upload/pertanggungjawaban_surat_tugas/' . $row->id . '.pdf') ? '<br><span class="badge bg-danger">laporan pertanggungjawaban belum diunggah</span>' : ''; ?>
                                        <?= file_exists('upload/dasar_penerbitan_surat_tugas/' . $row->id . '.pdf') ? '<br/><a href="' . base_url('upload/dasar_penerbitan_surat_tugas/' . $row->id . '.pdf') . '" target="__blank">Unduh dasar penerbitan</a>' : ''; ?>
                                        <?= file_exists('upload/pertanggungjawaban_surat_tugas/' . $row->id . '.pdf') ? '<br/><a href="' . base_url('upload/pertanggungjawaban_surat_tugas/' . $row->id . '.pdf') . '" target="__blank">Unduh laporan pertanggungjawaban</a>' : ''; ?>
                                    </td>
                                    <td><?= $row->nama; ?></td>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("surattugas/edit/" . $row->id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                                <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("surattugas/delete/" . $row->id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                            <?php endif ?>
                                            <a class="btn btn-success <?= $akses['download']; ?>" title="download" href="<?= base_url("surattugas/topdf/" . $row->id); ?>" target="__blank"><i class="fa-solid fa-eye"></i></a>
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <a class="btn btn-primary btn-sm <?= $akses['upload_pj']; ?>" title="Unggah Laporan Pertanggungjawaban" href="<?= base_url("/surattugas/upload/" . $row->id); ?>"><i class="fa-solid fa-upload"></i></a>
                                                ...
                                                <a class="btn btn-outline-primary btn-sm <?= $akses['share']; ?>" title="Bagikan" href="<?= base_url("surattugas/share/" . $row->id); ?>"><i class="fa-solid fa-share"></i></a>
                                            <?php endif ?>
...
<script>
    function viewpdf(id, akses) {
        ...
        $('#form-komentar').attr('action', "<?= site_url('surattugas/update/') ?>" + id);
        if (akses) {
            $('#preview-panel').show()
        } else {
            $('#preview-panel').hide()
        }
        var url = "<?= site_url('surattugas/topdf/'); ?>" + id
