        ...
        <div class="card-body">
            <form action="<?= site_url("p2m/p2m_dosen"); ?>" method="get">
                <div class="form-group row">
                    ...
                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                        <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('p2m/create_penerbit'); ?>">Tambah</a>
                    <?php } ?>
                </div>
            </form>
            ...
            <table class="table table-responsive table-striped table-bordered table-hover table-valign" style="vertical-align:middle;height: 60vh;">
                <thead>
                    <tr>
                        <th>No</th>
                        <th><?= $print_header('Nama', 'Publisher', $q); ?></th>
                        <th><?= $print_header('URL', 'url', $q); ?></th>
                        <th><?= $print_header('Level', 'levels', $q); ?></th>
                        <th><?= $print_header('Jurnal', 'journal', $q); ?></th>
                        <th><?= $print_header('Konferensi', 'conference', $q); ?></th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                        <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("p2m/edit_penerbit/" . $row->penerbit_id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                        <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("p2m/delete_penerbit/" . $row->penerbit_id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                    <?php endif ?>
                                </div>
                            </td>
                        </tr>
