            ...
            <form action="<?= site_url("p2m/p2m_dosen"); ?>" method="get">
                <div class="form-group row">
                    ...
                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                        <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('p2m/create_acadStaf'); ?>">Tambah</a>
                    <?php } ?>
                </div>
            </form>
            ...
            <table class="table table-responsive table-striped table-bordered table-hover table-valign" style="vertical-align:middle;height: 60vh;">
                <thead>
                    <tr>
                        ...
                        <th><?= $print_header('Laboratorium', 'laboratory', $q); ?></th>
                        <th><?= $print_header('Penelitian', 'researches', $q); ?></th>
                        <th><?= $print_header('Penelitian 2', 'researches_2', $q); ?></th>
                        <th><?= $print_header('Publikasi', 'publications', $q); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($rows as $row) : ?>
                        ...
                        <tr>
                            ...
                            <td><?= $row->researches; ?></td>
                            <td><?= $row->researches_2; ?></td>
                            <td><?= $row->publications; ?></td>
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                        <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("p2m/edit_acadStaf/" . $row->tendik_id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                        <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("p2m/delete_acadStaf/" . $row->tendik_id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                    <?php endif ?>
                                </div>
                            </td>
                        </tr>
