            ...
            <form action="<?= site_url("p2m/p2m_dosen"); ?>" method="get">
                <div class="form-group row">
                    ...
                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                        <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('p2m/create_konferensi'); ?>">Tambah</a>
                    <?php } ?>
                </div>
            </form>
            ...
            <table class="table table-responsive table-striped table-bordered table-hover table-valign" style="vertical-align:middle;height: 60vh;">
                <thead>
                    <tr>
                        ...
                        <th><?= $print_header('Organizer', 'organizer', $q); ?></th>
                        <th><?= $print_header('Lokasi Daerah', 'location_regency', $q); ?></th>
                        <th><?= $print_header('Lokasi Negara', 'location_country', $q); ?></th>
                        <th><?= $print_header('Tanggal Mulai', 'date_start', $q); ?></th>
                        <th><?= $print_header('Tanggal Berakhir', 'date_end', $q); ?></th>
                        <th><?= $print_header('URL', 'url', $q); ?></th>
                        <th><?= $print_header('Level', 'levels', $q); ?></th>
                        <th><?= $print_header('Sitasi Publikasi', 'publication_citations', $q); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($rows as $row) : ?>
                        ...
                        <tr>
                            ...
                            <td><?= $row->Name; ?></td>
                            <td><?= $row->organizer; ?></td>
                            <td><?= $row->location_regency; ?></td>
                            <td><?= $row->location_country ; ?></td>
                            <td><?= $row->date_start ; ?></td>
                            <td><?= $row->date_end ; ?></td>
                            <td><?= $row->url ; ?></td>
                            <td><?= $row->levels ; ?></td>
                            <td><?= $row->publication_citations ; ?></td>
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                        <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("p2m/edit_konferensi/" . $row->konferensi_id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                        <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("p2m/delete_konferensi/" . $row->konferensi_id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                    <?php endif ?>
                                </div>
                            </td>
                        </tr>
