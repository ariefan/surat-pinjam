            ...
            <form action="<?= site_url("p2m/p2m_dosen"); ?>" method="get">
                <div class="form-group row">
                    ...
                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                        <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('p2m/create_eksternal'); ?>">Tambah</a>
                    <?php } ?>
                </div>
            </form>
            ...
            <table class="table table-responsive table-striped table-bordered table-hover table-valign" style="vertical-align:middle;height: 60vh;">
                <thead>
                    <tr>
                        ...
                        <th><?= $print_header('Afiliasi', 'affiliations', $q); ?></th>
                        <th><?= $print_header('Negara', 'country', $q); ?></th>
                        <th><?= $print_header('Publikasi (sebagai co author)', 'publications_2_co_author', $q); ?></th>
                        <th><?= $print_header('Publikasi (sebagai main author)', 'publications_3_main_author', $q); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($rows as $row) : ?>
                        ...
                        <tr>
                            ...
                            <td><?= $row->country; ?></td>
                            <td><?= $row->publications_2_co_author; ?></td>
                            <td><?= $row->publications_3_main_author; ?></td>
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                        <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("p2m/edit_eksternal/" . $row->eksternal_id); ?>"><i class="fa-solid fa-pencil"></i></a>
                                        <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("p2m/delete_eksternal/" . $row->eksternal_id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                    <?php endif ?>
                                </div>
                            </td>
                        </tr>
