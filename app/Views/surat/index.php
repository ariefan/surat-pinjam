...
<div class="content">
    <div class="container-fluid">
        <div class="form-group row">
            <div class="col-lg-12">
                <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) { ?>
                    <!-- <a class="btn btn-success" title="Tambah" id="btnTambahST" href="<?= site_url('buatsurat/create'); ?>">Tambah</a> -->
        ...
        <form action="<?= site_url("buatsurat/index"); ?>" method="get">
            <div class="form-group row">
                <div class="col-sm-8">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    $print_header = function ($label, $column_name, $q) use ($sort_column, $sort_order) {
                        $url = site_url('buatsurat/index') . "?q=$q&sort_column=$column_name&sort_order=" . ($sort_order == 'asc' ? 'desc' : 'asc');
                    ...
                    <table class="table table-bordered table-valign-middle">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <a href="<?= site_url('buatsurat/index'); ?>?q=<?= $q; ?>&sort_column=nama_surat&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Nama Surat</a>
                                    <i class="pl-2 <?= $sort_column == 'nama_surat' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama_surat' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a href="<?= site_url('buatsurat/index'); ?>?q=<?= $q; ?>&sort_column=nama&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Pengaju</a>
                                    <i class="pl-2 <?= $sort_column == 'nama' ? '' : 'text-white'; ?> fa-solid fa-arrow-<?= $sort_column == 'nama' && $sort_order == 'desc' ? 'down' : 'up'; ?>"></i>
                                </th>
                                <th>
                                    <a href="<?= site_url('buatsurat/index'); ?>?q=<?= $q; ?>&sort_column=tanggal_pengajuan&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Tanggal Pengajuan</a>
                                ...
                                <th>
                                    <a href="<?= site_url('buatsurat/index'); ?>?q=<?= $q; ?>&sort_column=status&sort_order=<?= $sort_order == 'asc' ? 'desc' : 'asc'; ?>">Status</a>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a class="btn btn-warning btn-sm button-preview <?= $akses['preview']; ?>" title="Preview" href="<?= base_url("buatsurat/preview/" . $row->id); ?>" data-komentar="<?= $row->komentar; ?>" data-approve="<?= $akses['approve']; ?>" style="<?= !in_array($jenis_user, ['departemen']) ? '' : 'display:none;'; ?>"><i class="fa-sharp fa-solid fa-magnifying-glass"></i></a>
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <!-- <a class="btn btn-warning <?= $akses['edit']; ?>" title="edit" href="<?= base_url("buatsurat/edit/" . $row->id); ?>"><i class="fa-solid fa-pencil"></i></a> -->
                                                <a class="btn btn-danger <?= $akses['delete']; ?>" title="delete" href="<?= base_url("buatsurat/delete/" . $row->id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus surat ini?');"><i class="fa-solid fa-times"></i></a>
                                            <?php endif ?>
                                            <a class="btn btn-success <?= $akses['download']; ?>" title="download" href="<?= base_url("buatsurat/topdf/" . $row->id); ?>" target="__blank"><i class="fa-solid fa-eye"></i></a>
                                            <?php if (!in_array($jenis_user, ['dekan', 'wadek'])) : ?>
                                                <a class="btn btn-primary btn-sm <?= $akses['upload_pj']; ?>" title="Unggah Laporan Pertanggungjawaban" href="<?= base_url("/buatsurat/upload/" . $row->id); ?>"><i class="fa-solid fa-upload"></i></a>
                                                ...
                                                <a class="btn btn-outline-primary btn-sm <?= $akses['share']; ?>" title="Bagikan" href="<?= base_url("buatsurat/share/" . $row->id); ?>"><i class="fa-solid fa-share"></i></a> -->
                                                <a class="btn btn-outline-info btn-sm <?= $akses['copy']; ?>" title="Copy template" href="<?= base_url("buatsurat/copy/" . $row->id); ?>"><i class="fa-solid fa-copy"></i></a>
                                            <?php endif ?>
                                        </div><br>
                ...
                <div class="container">
                    <div class="row">
                        <?php foreach ($templates as $template) : ?>
                            <div class="col-sm-3">
                                <figure class="figure">
                                    <a href="<?= site_url('buatsurat/create/' . $template->id); ?>">
                                        <img src="<?= base_url('upload/surat_template_thumbnail/' . $template->id . '.png'); ?>" class="img-thumbnail m-2" style="width:120px;">
                                        <figcaption class="figure-caption text-center"><?= $template->nama_template; ?></figcaption>
                                    </a>
                                </figure>
                            </div>
                        <?php endforeach ?>
