...
<div class="card">
  ...
  <div class="card-body">
    <form action="<?= site_url('buatsurat/' . $action) . ($action == 'update' ? '/' . $row->id : '') ?>" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <input placeholder="Perihal Surat" autocomplete="off" type="text" class="form-control" name="nama_surat" value="<?= $row->nama_surat; ?>">
      </div>
      ...
      <div class="form-group">
        <label>Isi Surat</label><span style="color:black;"> (<i>Catatan: jika dalam pembuatan surat membutuhkan enter(ganti baris), maka tekan <b>shift+enter</b> (bukan <b>enter</b> saja)</i>)</span><span style="color:red;">*</span>
        <textarea class="form-control surat" id="open-source-plugins" name="isi_surat">
          <?= $row->isi_surat; ?>
        </textarea>
      </div>
        ...
        <div id="list-tembusan">
          <?php foreach ($row->tembusan as $tembusan) : ?>
            <div class="input-group input-group-sm mb-1 tembusan-item">
              <input type="text" class="form-control" placeholder="Tembusan" name="tembusan[]" value="<?= $tembusan; ?>">
      ...
      <div class="form-group">
        ...
        <?php if (file_exists('upload/dasar_penerbitan_surat/' . $row->id . '.pdf')) : ?>
          <a target="__blank" href="<?= base_url('upload/dasar_penerbitan_surat/' . $row->id . '.pdf') ?>">Dasar Penerbitan</a>
        <?php endif ?>
      </div>

      <div class="form-group">
        <label>Isi Lampiran</label><span style="color:black;"> (<i>Catatan: jika dalam pembuatan surat membutuhkan enter(ganti baris), maka tekan <b>shift+enter</b> (bukan <b>enter</b> saja)</i>)</span>
        <textarea class="form-control surat" id="open-source-plugins" name="isi_lampiran">
          <?= $row->isi_lampiran; ?>
        </textarea>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-sm-4">
          ...
          <button type="button" class="btn btn-warning" onclick="$('#modal-todo-buatsurat').modal('show'); $('#tugas').val($('#chat-input').val())">Todo list</button>
        </div>
      </div> -->
...
<div class="card" style="z-index:9999;width:300px;position:fixed;top:5px;right:5px;">
  <div class="card-header bg-warning">
    ...
    <button class="btn btn-sm btn-success py-0 float-right" data-toggle="modal" data-target="#modal-todo-buatsurat"><i class="fas fa-plus"></i></button>
  </div>
  <div class="card-body bg-warning" id="todo-body">
    <?php foreach ($rows as $row) : ?>
      <div class="form-check">
        <input id="todo-<?= $row->id; ?>" class="form-check-input todo-check" type="checkbox" data-id="<?= $row->id; ?>" value="1" <?= $row->status_tugas ? 'checked' : ''; ?>>
        <label class="form-check-label text-sm" for="todo-<?= $row->id; ?>" style="<?= $row->status_tugas ? 'text-decoration:line-through;' : ''; ?>" id="">
          <?= $row->tugas; ?><a class="btn btn-link" title="delete" href="<?= base_url("todo/delete/" . $row->id); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?');">hapus</a>
        </label>
        <p class="text-sm text-muted"><i class="far fa-clock m-0 p-0"></i> <?= $row->deadline; ?></p>
      </div>
    <?php endforeach; ?>
  </div>
</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('todo/store'); ?>" method="POST" enctype="multipart/form-data">
          <div class="form-group">
            <label>Pesan</label>
            <input class="form-control" type="text" id="tugas" name="tugas" placeholder="Masukkan deskripsi tugas" value="" autocomplete="off">
