    ...
    <div class="chat-widget-content">
        <div class="chat-widget-tabs">
            <div class="chat-widget-tab chat-widget-login-tab">
                <form action="<?= base_url('chat/authenticate'); ?>" method="post">
                    <input type="text" name="name" value="test@test.com" placeholder="Your Name">
                    <input type="email" name="email" value="test@test.com" placeholder="Your Email" required>
                    <div class="msg"></div>
...
setInterval(() => {
    ...
    if (currentChatTab == 2) {
        ...
        fetch('<?= base_url('chat/conversations'); ?>', { cache: 'no-store' }).then(response => response.text()).then(html => {
