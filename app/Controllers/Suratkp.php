...
class Suratkp extends BaseController
{
    private $pjs;
    private $st;
    ...
    public function index($count = false)
    {
        ...
        $db->query("UPDATE notifications SET status = 1 WHERE notification_type = 'surat_kp' AND user_id = '" . session('id') . "'");
        ...
        $this->st->select('surat_kp.*, users.nama, date(NOW()) <= tanggal_kegiatan_mulai IS NOT NULL as dalam_periode')
            ->join('users', 'surat_kp.user_id = users.id')
            ->join('pegawais', 'surat_kp.penandatangan_pegawai_id = pegawais.user_id', 'left')
            ->join('departemen', 'surat_kp.departemen_pegawai_id = departemen.kepala_pegawai_id', 'left')
            ->where("IF(surat_kp.user_id = $id OR (JSON_CONTAINS(shares, CONCAT('[', $id ,']')) AND status = 3) OR ('$jenis_user' IN('admin')) OR ('$jenis_user' IN('verifikator') AND '$gol_verifikator' = departemen.id) OR ('$jenis_user' IN('dekan', 'wadek') AND status >= 2 AND penandatangan_pegawai_id = $pegawai_id) OR ('$jenis_user' IN('departemen') AND status = 3 AND departemen_pegawai_id = $pegawai_id), true, false )")
        ...
        return view('surat-kp/index', $data);
    }

    //Proses input data surat baru
    public function create()
    {
        $row = new SuratKPModel();
        ...
        return view('surat-kp/form', $data);
    }
    ...
    public function store()
    {
        ...
        return $this->response->redirect(site_url('suratkp'));
        // return dump($data);
    }
    ...
    {
        $row = (new SuratKPModel)->where('id', $id)->first();
            ...
            switch ($data['status']) {
                ...
                case 2:
                    ...
                    if ($pegawai) {
                        ...
                        $email->setMessage('Surat dengan nomor: <a href="' . base_url("suratkp/topdf/$id") . '">' . $row->no_surat . '</a> menunggu verifikasi anda.');
                        if (!$email->send())
                ...
                case 3:
                    ...
                    $email->setSubject('Verifikasi Surat Baru');
                    $email->setMessage('Surat dengan nomor: <a href="' . base_url("suratkp/topdf/$id") . '">' . $row->no_surat . '</a> sudah terverifikasi.');
                    if (!$email->send())
                        echo $email->printDebugger(['headers']);
                    break;
                default:
    ...
    public function delete($id)
    {
        ...
        (new SuratKPModel)->where('id', $id)->delete();
        if (file_exists(FCPATH . "upload/surat_kp/$id.pdf"))
            unlink(FCPATH . "upload/surat_kp/$id.pdf");
        return $this->response->redirect(site_url('suratkp'));
    }
    ...
    //     (new SuratKPModel())->update($id, ['file_pertanggungjawaban' => $name . '.pdf']);
    //     session()->setFlashdata('success', 'Berkas berhasil diupload');
    //     return $this->response->redirect(site_url('suratkp'));
    // }
    ...
    public function share($id)
    {
        ...
        return view('surat-kp/share', $data);
    }
    ...
    public function topdf($id, $save = false)
    {
        ...
        // return view('surat-kp/print', $data);
        // exit;

        $dompdf->loadHtml(view('surat-kp/print', $data));
