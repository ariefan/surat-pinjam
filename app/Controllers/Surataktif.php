...
use App\Models\SuratAktifModel;
...
class Surataktif extends BaseController
{
    private $pjs;
    private $st;
    ...
    {
        $this->st = new SuratAktifModel();

        $pjs = [];
    }

    //Ketika tampilan awal menu surat aktif
    public function index($count = false)
    {
        // Dismiss Notifikasi
        $db = \Config\Database::connect();
        $db->query("UPDATE notifications SET status = 1 WHERE notification_type = 'surat_aktif' AND user_id = '" . session('id') . "'");
        ...
        $this->st->select('surat_aktif.*, users.nama, date(NOW())')
            ->join('users', 'surat_aktif.user_id = users.id')
            ->join('pegawais', 'surat_aktif.penandatangan_pegawai_id = pegawais.user_id', 'left')
            ->join('departemen', 'surat_aktif.departemen_pegawai_id = departemen.kepala_pegawai_id', 'left')
            ->where("IF(surat_aktif.user_id = $id OR (JSON_CONTAINS(shares, CONCAT('[', $id ,']')) AND status = 3) OR ('$jenis_user' IN('admin')) OR ('$jenis_user' IN('verifikator') AND '$gol_verifikator' = departemen.id) OR ('$jenis_user' IN('dekan', 'wadek') AND status >= 2 AND penandatangan_pegawai_id = $pegawai_id) OR ('$jenis_user' IN('departemen') AND status = 3 AND departemen_pegawai_id = $pegawai_id), true, false )")
        ...
        return view('surat-aktif/index', $data);
    }

    //Proses input data surat baru
    public function create()
    {
        $row = new SuratAktifModel();
        ...
        return view('surat-aktif/form', $data);
    }
    ...
    public function store()
    {
        ...
        $db->query("INSERT INTO notifications
            select null, id, 'surat_aktif', 'surat', '0', now(), now() from users 
        ...
        return $this->response->redirect(site_url('surataktif'));
    }

    //Proses edit data surat baru
    public function edit($id)
    {
        $row = (new SuratAktifModel)->where('id', $id)->first();
    ...
    public function update($id)
    {
        ...
        if (isset($data['status'])) {
            switch ($data['status']) {
                ...
                case 2:
                    $db->query("INSERT INTO notifications
                        select null, users.id, 'surat_aktif', 'surat', '0', now(), now() from users 
                ...
                case 3:
                    ...
                    $email->setMessage('Surat dengan nomor: <a href="' . base_url("surataktif/topdf/$id") . '">' . $row->no_surat . '</a> sudah terverifikasi.');
                    if (!$email->send())
    ...
    public function delete($id)
    {
        ...
        (new SuratAktifModel)->where('id', $id)->delete();
        if (file_exists(FCPATH . "upload/surat_aktif/$id.pdf"))
            unlink(FCPATH . "upload/surat_aktif/$id.pdf");
        return $this->response->redirect(site_url('surataktif'));
    }
    ...
    {
        $db = \Config\Database::connect();
        $data['surat_aktif'] = $db->query("SELECT
        nama, nomor, pangkat
        FROM users
        JOIN surat_aktif ON user_id=users.id")->getResult();
    ...
    {
        return view('surat-aktif/upload', ['id' => $id]);
    }

    public function uploads($id)
    {
        if (!empty($this->request->getFile('berkas')->getFilename())) {
            if (file_exists("upload/pertanggungjawaban_surat_aktif/$id.pdf"))
                unlink("upload/pertanggungjawaban_surat_aktif/$id.pdf");
            $file = $this->request->getFile('berkas');
            $file->move('upload/pertanggungjawaban_surat_aktif', $id . '.pdf');
        }
    ...
    public function share($id)
    {
        ...
        return view('surat-aktif/share', $data);
    }
    ...
    public function upload_dasar_penerbitan($id)
    {
        if (!empty($this->request->getFile('berkas')) && !empty($this->request->getFile('berkas')->getFileName())) {
            if (file_exists("upload/dasar_penerbitan_surat_aktif/$id.pdf"))
                unlink("upload/dasar_penerbitan_surat_aktif/$id.pdf");
            $file = $this->request->getFile('berkas');
            $file->move('upload/dasar_penerbitan_surat_aktif', $id . '.pdf');
        }
    }

    public function topdf($id, $save = false)
    {
        $filename = date('y-m-d_H.i.s') . '-surat_aktif';
        ...
        no_surat, nama_mhs, nim, prog_studi, surat_aktif.departemen, tanggal_pengajuan, status, tembusan, keperluan, tahun_ajaran, semester
        FROM users
        JOIN pegawais ON users.id = pegawais.user_id
        JOIN surat_aktif ON surat_aktif.user_id=pegawais.user_id
        WHERE surat_aktif.id = '$id'")->getResult()[0];
        ...
        FROM surat_aktif
        JOIN pegawais ON surat_aktif.penandatangan_pegawai_id = pegawais.id
        JOIN penandatangan ON pegawais.id = penandatangan.pegawai_id
        WHERE surat_aktif.id = '$id'")->getResult();
        ...
        $dompdf->loadHtml(view('surat-aktif/print', $data));
        ...
        // return $this->response->redirect(site_url('surattugas'));
        if ($save) {
            file_put_contents("upload/surat_aktif/$id.pdf", $dompdf->output());
