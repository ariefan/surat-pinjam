...
use App\Models\SuratBandosModel;
...
class Suratbandos extends SuratController
{
    private $st;

    function __construct()
    {
        $this->st = new SuratBandosModel();
    }
    ...
    public function index($count = false)
    {
        ...
        $this->st->select('surat_bandos.*, users.nama, dp.kepala_pegawai_id, dp.sekretaris_pegawai_id, p.departemen departemen_pengusul')
            ->join('users', 'surat_bandos.user_id = users.id')
            ->join('pegawais', 'surat_bandos.penandatangan_pegawai_id = pegawais.user_id', 'left')
            ->join('pegawais AS p', 'surat_bandos.user_id = p.user_id', 'left')
            ->join('departemen', 'surat_bandos.departemen_pegawai_id = departemen.kepala_pegawai_id', 'left')
            ->join('departemen AS dp', 'p.departemen_id = dp.id', 'left')
            ->where("
                IF(surat_bandos.user_id = $id 
                ...
                OR ('$jenis_user' IN('dekan', 'wadek') AND status >= 2 ) 
        ...
        return view('surat-bandos/index', $data);
    }

    //Proses input data surat baru
    public function create($tipe_surat = 'permohonan')
    {
        $row = new SuratBandosModel();
        $row->tanggal_pengajuan = date('Y-m-d');
        $row->tembusan = [];
        $row->departemens = (new DepartemenModel)->where('departemen.id < 5')->get()->getResult();
        $row->tipe_surat = strtoupper($tipe_surat);
        $data = [
            'action' => 'store',
            'row' => $row,
        ...
        return view('surat-bandos/form', $data);
    }
    ...
    public function store()
    {
        ...
        $data['nama_surat'] = $data['departemen_pembuat'] . ' ' . strtolower($data['tipe_surat']) . ' ke ' . $data['fakultas_surat_tindaklanjut'];
        //$data['tembusan'] = json_encode($data['tembusan'] ?? []);
            ...
            foreach ([$d->kepala_pegawai_id, $d->sekretaris_pegawai_id] as $ids) {
                if (!empty($ids)) {
                    ...
                    $db->query("INSERT INTO notifications
                        select null, users.id, 'surat_bandos', 'surat', '0', now(), now() from users 
                    ...
                    $data_todo = [
                        ...
                        'user_id' => $p->user_id,
                        'link' => 'https://fmipaugm.id/suratbandos?preview_id=' . $id . '#'
                    ];
        ...
        $db = \Config\Database::connect();
        $db->query("INSERT INTO notifications
            select null, id, 'surat_bandos', 'surat', '0', now(), now() from users 
        ...
        return $this->response->redirect(site_url('suratbandos'));
    }

    //Proses edit data surat baru
    public function edit($id)
    {
        $row = (new SuratBandosModel)->where('id', $id)->first();
        $row->tembusan = json_decode($row->tembusan) ?? [];
        $row->departemens = (new DepartemenModel)->where('departemen.id < 5')->get()->getResult();
        $data = [
            'action' => 'update',
            'row' => $row,
            'departemens' => (new DepartemenModel)->join('pegawais', 'pegawais.id = departemen.kepala_pegawai_id')->where('departemen.id > 4')->get()->getResult(),
        ];
        return view('surat-bandos/form', $data);
    }
    ...
    public function update($id)
    {
        ...
        if (!empty($data['departemen_pembuat'])) {
            $data['nama_surat'] = $data['departemen_pembuat'] . ' ' . strtolower($data['tipe_surat']) . ' ke ' . $data['fakultas_surat_tindaklanjut'];
        }
        ...
        $row = $this->st
            ->join('users', 'surat_bandos.user_id = users.id')
            ->where('surat_bandos.id', $id)->first();
        $db = \Config\Database::connect();
        if (isset($data['status'])) {
            switch ($data['status']) {
                case -1:
                ...
                case 2:
                    ...
                    if ($pegawai) {
                        ...
                        $email->setMessage('Surat <a href="' . base_url("suratbandos") . "?preview_id=$id" . '">' . $row->nama_surat . '</a> menunggu verifikasi anda.');
                        if (!$email->send())
                ...
                case 3:
                    ...
                    (new DetailNomorSuratModel())->insert([
                        ...
                        'tujuan_surat' => "Surat Bantuan Dosen",
                        'sifat_surat' => "1",
                        'user_id' => $row->user_id
                    ]);
                    $db->query("INSERT INTO notifications
                    ...
                    // $email->setSubject('Verifikasi Surat Baru');
                    // $email->setMessage('Surat <a href="' . base_url("suratbandos") . "?preview_id=$id" . '">' . $row->nama_surat . '</a> sudah terverifikasi.');
                    // if (!$email->send())
                    //     echo $email->printDebugger(['headers']);
                    break;
                default:
    ...
    public function delete($id)
    {
        ...
        (new SuratBandosModel)->where('id', $id)->delete();
        if (file_exists(FCPATH . "upload/surat_bandos/$id.pdf"))
            unlink(FCPATH . "upload/surat_bandos/$id.pdf");
        return $this->response->redirect(site_url('suratbandos'));
    }
    ...
    {
        $db = \Config\Database::connect();
        $data['surat_bandos'] = $db->query("SELECT
        nama, nomor, pangkat
        FROM users
        JOIN surat_bandos ON user_id=users.id")->getResult();
    ...
    {
        return view('surat-bandos/upload', ['id' => $id]);
    }

    public function uploads($id)
    {
        if (!empty($this->request->getFile('berkas')->getFilename())) {
            if (file_exists("upload/pertanggungjawaban_surat_bandos/$id.pdf"))
                unlink("upload/pertanggungjawaban_surat_bandos/$id.pdf");
            $file = $this->request->getFile('berkas');
            $file->move('upload/pertanggungjawaban_surat_bandos', $id . '.pdf');
        }
    ...
    public function share($id)
    {
        ...
        return view('surat-bandos/share', $data);
    }
    ...
    {
        $filename = date('y-m-d_H.i.s') . '-surat_bandos';
        $dompdf = new Dompdf();
        ...
        no_surat, nama, nomor, pegawais.pangkat, tanggal_pengajuan, tipe_surat, fakultas_surat_tindaklanjut, jabatan_surat_tindaklanjut,
        no_surat_tindaklanjut, tanggal_surat_tindaklanjut, status,
        pegawais.nip nip, tabel, tembusan, paragraf_baru, departemen_pembuat, tanggal_deadline
        FROM users
        JOIN pegawais ON users.id = pegawais.user_id
        JOIN surat_bandos ON surat_bandos.user_id=pegawais.user_id
        WHERE surat_bandos.id = '$id'")->getResult()[0];
        ...
        FROM surat_bandos 
        JOIN pegawais ON surat_bandos.penandatangan_pegawai_id = pegawais.id
        JOIN penandatangan ON pegawais.id = penandatangan.pegawai_id
        WHERE surat_bandos.id = '$id'")->getResult();
        ...
        $html = view('surat-bandos/printlampiran', $data);
        ...
        $dompdfUtama = new Dompdf();
        $dompdfUtama->getOptions()->setChroot(FCPATH);
        $dompdfUtama->loadHtml(view('surat-bandos/print', $data));
        $dompdfUtama->setPaper('A4', 'portrait');
        $dompdfUtama->render();
        $m->addRaw($dompdfUtama->output());
        $m->addRaw($dompdf->output());
        $createdPdf = $m->merge();
        if ($save) {
            file_put_contents("upload/surat_bandos/$id.pdf", $createdPdf);
            file_put_contents("validasi/$id.pdf", $createdPdf);
        } else {
            // return view('surat-bandos/printlampiran', $data);
