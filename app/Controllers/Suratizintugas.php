...
use App\Models\SuratIzinTugasModel;
...
class Suratizintugas extends SuratController
{
    private $pjs;
    private $st;
    ...
    {
        $this->st = new SuratIzinTugasModel();
        $pjs = [];
    }
    ...
    public function index($count = false)
    {
        ...
        $db->query("UPDATE notifications SET status = 1 WHERE notification_type = 'surat_izin_tugas' AND user_id = '" . session('id') . "'");
        ...
        $this->st->select('surat_izin_tugas.*, users.nama, dp.kepala_pegawai_id, dp.sekretaris_pegawai_id, p.departemen departemen_pengusul, date(NOW()) <= tanggal_kegiatan_mulai OR file_pertanggungjawaban IS NOT NULL as dalam_periode')
            ->join('users', 'surat_izin_tugas.user_id = users.id')
            ->join('pegawais', 'surat_izin_tugas.penandatangan_pegawai_id = pegawais.user_id', 'left')
            ->join('pegawais AS p', 'surat_izin_tugas.user_id = p.user_id', 'left')
            ->join('departemen', 'surat_izin_tugas.departemen_pegawai_id = departemen.kepala_pegawai_id', 'left')
            ->join('departemen AS dp', 'p.departemen_id = dp.id', 'left')
            ->where("
                IF(surat_izin_tugas.user_id = $id 
        ...
        return view('surat-izin-tugas/index', $data);
    }

    //Proses input data surat baru
    public function create()
    {
        $row = new SuratIzinTugasModel();
        ...
        return view('surat-izin-tugas/form', $data);
    }
    ...
    public function store()
    {
            ...
            foreach ([$d->kepala_pegawai_id, $d->sekretaris_pegawai_id] as $ids) {
                if (!empty($ids)) {
                    ...
                    $db->query("INSERT INTO notifications
                        select null, users.id, 'surat_izin_tugas', 'surat', '0', now(), now() from users 
                    ...
                    $data_todo = [
                        ...
                        'user_id' => $p->user_id,
                        'link' => 'https://fmipaugm.id/suratizintugas?preview_id=' . $id . '#'
                    ];
        ...
        $db = \Config\Database::connect();
        $db->query("INSERT INTO notifications
            select null, id, 'surat_izin_tugas', 'surat', '0', now(), now() from users 
        ...
        return $this->response->redirect(site_url('suratizintugas'));
    }

    //Proses edit data surat baru
    public function edit($id)
    {
        $row = (new SuratIzinTugasModel)->where('id', $id)->first();
    ...
    public function update($id)
    {
        ...
        $row = $this->st
            ->join('users', 'surat_izin_tugas.user_id = users.id')
            ->where('surat_izin_tugas.id', $id)->first();
        $db = \Config\Database::connect();
        if (isset($data['status'])) {
            switch ($data['status']) {
                case -1:
                ...
                case 2:
                    ...
                    if ($pegawai) {
                        ...
                        $email->setMessage('Surat <a href="' . base_url("suratizintugas") . "?preview_id=$id" . '">' . $row->nama_surat . '</a> menunggu verifikasi anda.');
                ...
                case 3:
                    ...
                    $email->setSubject('Verifikasi Surat Baru');
                    $email->setMessage('Surat <a href="' . base_url("suratizintugas") . "?preview_id=$id" . '">' . $row->nama_surat . '</a> sudah terverifikasi.');
                    if (!$email->send()) echo $email->printDebugger(['headers']);
                    break;
                default:
                    break;
    ...
    public function delete($id)
    {
        ...
        (new SuratIzinTugasModel)->where('id', $id)->delete();
        if (file_exists(FCPATH . "upload/surat_izin_tugas/$id.pdf")) unlink(FCPATH . "upload/surat_izin_tugas/$id.pdf");
        return $this->response->redirect(site_url('suratizintugas'));
    }
    ...
    {
        $db = \Config\Database::connect();
        $data['surat_izin_tugas'] = $db->query("SELECT
        nama, nomor, pangkat
        FROM users
        JOIN surat_izin_tugas ON user_id=users.id")->getResult();
    ...
    {
        return view('surat-izin-tugas/upload', ['id' => $id]);
    }

    public function uploads($id)
    {
        if (!empty($this->request->getFile('berkas')->getFilename())) {
            if (file_exists("upload/pertanggungjawaban_surat_izin_tugas/$id.pdf")) unlink("upload/pertanggungjawaban_surat_izin_tugas/$id.pdf");
            $file = $this->request->getFile('berkas');
            $file->move('upload/pertanggungjawaban_surat_izin_tugas',  $id . '.pdf');
        }
    ...
    // {
    //     $db = \Config\Database::connect();
    //     $result = $db->query("SELECT file_pertanggungjawaban, date(NOW()) BETWEEN tanggal_kegiatan_mulai AND tanggal_kegiatan_selesai as dalam_periode FROM surat_izin_tugas WHERE id = '$id'")->getResult();

    //     $old_name = $result[0]->file_pertanggungjawaban;
    //     if (file_exists("upload/pertanggungjawaban_surat_izin_tugas/$old_name.pdf")) unlink("upload/pertanggungjawaban_surat_izin_tugas/$old_name.pdf");

    //     $file = $this->request->getFile('berkas');
    //     $name = hash_file('ripemd160', $file);
    //     $file->move('upload/pertanggungjawaban_surat_izin_tugas',  $name . '.pdf');

    //     (new SuratIzinTugasModel())->update($id, ['file_pertanggungjawaban' => $name . '.pdf']);
    //     session()->setFlashdata('success', 'Berkas berhasil diupload');
    //     return $this->response->redirect(site_url('suratizintugas'));
    // }
    ...
    public function share($id)
    {
        ...
        return view('surat-izin-tugas/share', $data);
    }
    ...
    public function upload_dasar_penerbitan($id)
    {
        if (!empty($this->request->getFile('berkas')) && !empty($this->request->getFile('berkas')->getFileName())) {
            if (file_exists("upload/dasar_penerbitan_surat_izin_tugas/$id.pdf")) unlink("upload/dasar_penerbitan_surat_izin_tugas/$id.pdf");
            $file = $this->request->getFile('berkas');
            $file->move('upload/dasar_penerbitan_surat_izin_tugas',  $id . '.pdf');
        }
    }

    public function topdf($id, $save = false)
    {
        $filename = date('y-m-d_H.i.s') . '-surat_izin_tugas';
        ...
        nama_surat, lokasi_kegiatan, status, berdasarkan_surat, berdasarkan_nomor_surat,
        pegawais.nip nip, tabel, tembusan, paragraf_baru
        FROM users
        JOIN pegawais ON users.id = pegawais.user_id
        JOIN surat_izin_tugas ON surat_izin_tugas.user_id=pegawais.user_id
        WHERE surat_izin_tugas.id = '$id'")->getResult()[0];
        ...
        FROM surat_izin_tugas 
        JOIN pegawais ON surat_izin_tugas.penandatangan_pegawai_id = pegawais.id
        JOIN penandatangan ON pegawais.id = penandatangan.pegawai_id
        WHERE surat_izin_tugas.id = '$id'")->getResult();
        ...
        // return view('surat-izin-tugas/print', $data);
        // exit;

        $dompdf->loadHtml(view('surat-izin-tugas/print', $data));
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
        // $output = $dompdf->output();
        // file_put_contents("upload/surat_izin_tugas/" . $id . ".pdf", $output);          
        // return $this->response->redirect(site_url('suratizintugas'));
        if ($save) {
            file_put_contents("upload/surat_izin_tugas/$id.pdf", $dompdf->output());
